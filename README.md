...
`rss2discord_feed_request_duration_seconds` is exposed as a hybrid classic+native histogram: the classic `le` buckets that the bundled dashboard queries are unchanged, and with `prometheus.scrapeNativeHistograms` (default `true`, requires `prometheus.enabled`) the `ServiceMonitor` additionally sets `scrapeProtocols`/`scrapeClassicHistograms`/`scrapeNativeHistograms` on the `ServiceMonitor`'s `spec` (these are top-level `ServiceMonitorSpec` fields, not per-endpoint) so Prometheus negotiates protobuf and scrapes the higher-resolution native representation alongside the classic one. Set `prometheus.scrapeNativeHistograms=false` to scrape classic buckets only.

To actually use the native histogram once it's being scraped:

- **Prometheus must support it too.** Native histograms require Prometheus >= v2.45, and are only stable (no feature flag needed) since v3.8; on v2.45–v3.7 you also need to start Prometheus with `--enable-feature=native-histograms`. Without this, Prometheus drops the native representation even though the `ServiceMonitor` requests it.
- **Query it directly by metric name** — unlike classic histograms there's no `_bucket`/`_count`/`_sum` suffix or `le` label to sum over. `histogram_quantile(0.95, rate(rss2discord_feed_request_duration_seconds[5m]))` against the bare metric name gives you the native-histogram p95, with much finer resolution than the classic buckets defined in [metrics.go](internal/controller/metrics.go). Compare against the classic query `histogram_quantile(0.95, sum(rate(rss2discord_feed_request_duration_seconds_bucket[5m])) by (le))` — both work simultaneously since the metric is hybrid.
- **Grafana needs a recent-enough version** (>= v9.4, ideally >= v11 for heatmap support) with the Prometheus data source's native histogram support enabled to render the "Performance (Native Histograms)" row the bundled dashboard ships; older Grafana will show the panels empty rather than erroring.
